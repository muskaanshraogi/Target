...
const getTeacherSubjects = (reg_id, callback) => {
    db.query(
        "SELECT subject.subId, subject.subName, subject.year, faculty.role_id FROM subject JOIN faculty ON subject.subId=faculty.subId WHERE subject.subId IN (SELECT subId FROM faculty WHERE reg_id=?)",
        [reg_id],
        (err, res) => {
            if(err) {
                return callback(err, 500, null)
