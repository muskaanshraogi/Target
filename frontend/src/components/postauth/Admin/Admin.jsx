...
export default function Admin() {
  ...
  return (
    <div className={classes.root}>
      <div style={{ float: 'right', display: 'inline-flex block', width: '36%', marginBottom: '1%' }}>
        <Select
          style={{ width: '22%' }}
        ...
        >
          ...
          {
            year.map((y) => (
              <option value={y.acadYear} style={{ padding: '3px' }}>{y.acadYear}</option>
            ))
          }
        </Select>
        <Button
         ...
         variant='contained'
         color='primary'
         onClick={() => { setClearYear(true) }}>
          Clear Academic Year
        </Button>
        <Button
         className={classes.buttonDg}
         variant='contained'
         onClick={() => { setResetDb(true) }}>
          Reset Database
        </Button>
      </div>
      <Tabs
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {
              clearYear ? <p>Are you sure you want to delete all data for the academic year <b>{acadYear}</b>?</p> : null
            }
            {
              resetDb ? 
                <p>Are you sure you want to reset the database? It will permanently delete all your staff and subject records.</p> : 
                null
            }
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose} variant="primary" style={{ paddingTop: '8px'}}>
            Cancel
          </Button>
          <Button onClick={handleDelete} variant='primary' style={{ backgroundColor: '#f50057', color: '#ffffff' }} >
            Delete
          </Button>
        </DialogActions>
      </Dialog>
